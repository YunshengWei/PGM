...
numAlleles = zeros(size(geneCopyVarOneList'));
...
for i = 1:length(phenotypeFactor.val)
    ...
    for j = 1:length(geneCopyVarOneList)
        z = z + alleleWeights{j}(assignment(j + 1));
    end
    for j = 1:length(geneCopyVarTwoList)
        z = z + alleleWeights{j}(assignment(j + length(geneCopyVarOneList) + 1));
    end
    if assignment(1) == 1
