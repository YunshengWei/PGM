...
for i = 1:size(genotypesToAlleles, 1)
    p11 = genotypesToAlleles(i, 1);
    p12 = genotypesToAlleles(i, 2);
    for j = 1:size(genotypesToAlleles, 1)
        p21 = genotypesToAlleles(j, 1);
        p22 = genotypesToAlleles(j, 2);
        assignments = [allelesToGenotypes(p11, p21), i, j;
                       allelesToGenotypes(p11, p22), i, j;
                       allelesToGenotypes(p12, p21), i, j;
                       allelesToGenotypes(p12, p22), i, j];
        ind = AssignmentToIndex(assignments, genotypeFactor.card);
        for k = 1:length(ind)
            genotypeFactor.val(ind(k)) = genotypeFactor.val(ind(k)) + 0.25;
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%